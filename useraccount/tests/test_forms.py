...
class UserTestCast(TestCase):
    ...
    def setUp(self):
        self.user_a = User(email='cfe@invalid.com')
    ...
    def test_new_user_added_is_created(self):
        self.assertEqual(len(User.objects.filter(email="bobby@test.com")), 0)

        bobby = User(email="bobby@test.com")
        bobby.save()

        return_user = User.objects.filter(email="bobby@test.com")
